...
class ModuleToLean4:
    ...
    def atom_to_lean_bool_rel(self) -> str:
        """Generate a Bool-valued relational encoding inside ``namespace ScalarBoolRel``.

        Like ``atom_to_lean_rel`` (ScalarRel) but:
        - Every relation/condition returns ``Bool`` (``==`` and ``&&`` instead of ``=`` and ``∧``)
        - State and input variables are declared with ``variable`` at the top of the
          namespace rather than listed on each ``def``; Lean auto-inserts them as
          explicit parameters on any definition whose body mentions them.
        """
        ctx = self.ctx
        noncomp = "noncomputable " if ctx.uses_real else ""
        ...
        lines = ["namespace ScalarBoolRel", ""]

        has_update = bool(list(ctx.atom.update))
        has_init = bool(list(ctx.atom.init))

        # Determine, at the wire level, which input groups each effect/init actually
        # consumes.  We collect the read-wire IDs of the reachable terms and check
        # overlap with each boundary group.  This is used to build exact call-arg
        # lists so that callers don't pass variables that weren't auto-inserted as
        # parameters (which would produce "Function expected" errors).
        ...
        # Variable declarations: all candidates declared once, Lean inserts only
        # those that appear free in each definition's body.
        # `abbrev` (@[reducible]) keeps definitions transparent to tactics.
        var_parts = []
        ...
        if has_update:
            # Compute effect bodies and consumed-arg lists for all effects.
        ...
        if has_init:
            # Compute init bodies and consumed-arg lists for all inits.
        ...
        lines.append("end ScalarBoolRel")
        return "\n".join(lines)
