...
class Context(ContextBase):
    ...
    def get_pyval_sym(self, sym: Expr) -> PyVal:
        ...
        return self._context.get_sym(sym.symbol_name(), from_pysmt_type(sym.symbol_type()))
    ...
    def to_terms(self, ctrl, extl, init_ret, update_ret) -> (list, list, list, list):
        ...
        for var, expr in zip(ctrl, init_ret):
            ...
            assert len(outvar) == 1
            init_terms.append(WrappedTerm("Id", outvar, [self.get_pyval_sym(self.next_var(var))]))
        ...
        for var, expr in zip(ctrl, update_ret):
            ...
            # map the output of the expression to the output wire
            assert len(outvar) == 1
            update_terms.append(WrappedTerm("Id", outvar, [self.get_pyval_sym(self.next_var(var))]))
...
class ToTerms:
    ...
    def walk(self, formula, depth=0):
        ...
        if isinstance(formula, Case):
            ...
            if opty == op.SYMBOL:
                return [self._ctx.get_sym(formula.symbol_name(), from_pysmt_type(formula.symbol_type()))]
            elif opty == op.PLUS:
                assert len(args) == 2
