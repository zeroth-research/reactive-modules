...
@[simp] def init (extl_n: Unit) : (Mat Int 1 1) × (Mat Int 1 1) :=
  ...
  let x1 : (Mat Int 1 1) := (fun _ _ => (10 : Int))
  let x2 : (Mat Int 1 1) := x0
  let x3 : (Mat Int 1 1) := x1
  (x2, x3)

@[simp] def update (ctrl: (Mat Int 1 1) × (Mat Int 1 1)) (extl_l: Unit) (extl_n: Unit) : (Mat Int 1 1) × (Mat Int 1 1) :=
...
def inv : (Mat Int 1 1) × (Mat Int 1 1) → Prop := fun s => (((s.1 0 0) ≥ 0) ∧ ((s.1 0 0) ≤ (s.2 0 0)) ∧ ((s.2 0 0) = 10))

def P : (Mat Int 1 1) × (Mat Int 1 1) → Prop := fun s => ((s.1 0 0) = (s.2 0 0))

instance : DecidablePred P := fun s => by unfold P; first | infer_instance | dsimp; infer_instance

def ranking : (Mat Int 1 1) × (Mat Int 1 1) → Nat := fun s => (((if ((s.1 0 0) = (s.2 0 0)) then 0 else ((s.2 0 0) - (s.1 0 0))) : Int)).toNat


def RM : ReactiveModule ((Unit) × (Unit)) ((Mat Int 1 1) × (Mat Int 1 1)) := {
    init := fun e => init e.2
