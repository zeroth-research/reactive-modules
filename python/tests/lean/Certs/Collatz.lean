...
@[simp] def init (extl_n: Unit) : (Mat Int 1 1) :=
  let x0 : (Mat Int 1 1) := (fun _ _ => (7 : Int))
  let x1 : (Mat Int 1 1) := x0
  x1

@[simp] def update (ctrl: (Mat Int 1 1)) (extl_l: Unit) (extl_n: Unit) : (Mat Int 1 1) :=
  let x0 : (Mat Int 1 1) := (fun _ _ => (1 : Int))
  let x1 : (Mat Bool 1 1) := (fun _ _ => decide (ctrl 0 0 = x0 0 0))
  let x2 : (Mat Int 1 1) := (fun _ _ => (7 : Int))
  let x3 : (Mat Int 1 1) := (fun _ _ => (4 : Int))
  let x4 : (Mat Bool 1 1) := (fun _ _ => decide (x3 0 0 < ctrl 0 0))
  let x5 : (Mat Int 1 1) := (fun _ _ => (3 : Int))
  let x6 : (Mat Int 1 1) := (ctrl - x5)
  let x7 : (Mat Int 1 1) := (fun _ _ => (1 : Int))
  let x8 : (Mat Bool 1 1) := (fun _ _ => decide (x7 0 0 < ctrl 0 0))
  let x9 : (Mat Int 1 1) := (fun _ _ => (1 : Int))
  let x10 : (Mat Int 1 1) := (ctrl - x9)
  let x11 : (Mat Int 1 1) := (if x8 0 0 then x10 else ctrl)
  let x12 : (Mat Int 1 1) := (if x4 0 0 then x6 else x11)
  let x13 : (Mat Int 1 1) := (if x1 0 0 then x2 else x12)
  let x14 : (Mat Int 1 1) := x13
  x14
...
def inv : (Mat Int 1 1) → Prop := fun s => (((s 0 0) ≥ 1) ∧ ((s 0 0) ≤ 8))

def P : (Mat Int 1 1) → Prop := fun s => ((s 0 0) = 1)

instance : DecidablePred P := fun s => by unfold P; first | infer_instance | dsimp; infer_instance

def ranking : (Mat Int 1 1) → Nat := fun s => (((if ((s 0 0) = 1) then 0 else ((s 0 0) - 1)) : Int)).toNat
