...
pub fn to_lean_from_module(m: &Module<DType, IType>) -> Result<String, &'static str> {
    ...
    let obs = m.obs();
    let latched_wires: Vec<_> = obs.iter().next().map(|arr| arr.to_vec()).unwrap_or_default();
    ...
    for wire in latched_wires.iter() {
        if seen.insert(wire.id()) {
            wires.push((format!("x{}", wire.id()), wire.id(), *wire.dtype()));
        }
    }
    // Infer var_count as total latched wires minus external latched wires
    let extl_len = m.extl().wires().count();
    ...
    for atom in atoms.iter() {
        for t in atom.init().iter() {
            for wire in t.write().wires() {
                init_term_map.insert(wire.id(), t);
            }
        }
        for t in atom.update().iter() {
            for wire in t.write().wires() {
                update_term_map.insert(wire.id(), t);
            }
        }
    }
        ...
        if let Some(t) = term_map.get(&idx) {
            match t.itype() {
                ...
                Assign => {
                    ...
                    if let Some(wire) = t.read().wires().next() {
                        let ridx = wire.id();
                ...
                Lt | Le | Gt | Ge | Eq => {
                    ...
                    let mut wires_iter = t.read().wires();
                    if let Some(wire) = wires_iter.next() {
                        let lidx = wire.id();
    ...
    for (widx, t) in init_term_map.iter() {
        ...
        if widx >= n {
                ...
                && let IType::Assign = t.itype()
            {
                ...
                if let Some(wire) = t.read().wires().next()
                    && let Some(abs_term) = init_term_map.get(&wire.id())
                    && let IType::Abs = abs_term.itype()
                {
                    // Abs read should point at a primed IVAR index
                    if let Some(abs_wire) = abs_term.read().wires().next() {
                        let abs_ridx = abs_wire.id();
...
pub fn render_terms_to_lean(
            ...
            Cond => {
                ...
                if args.len() >= 3 {
                        ...
                        && let Lt = cond_term.itype()
                    {
                        ...
                        for wire in cond_term.read().wires() {
                let ridx = wire.id();
                            carr.push(ridx);
                        }
                        if carr.len() >= 2 {
    ...
    render_idx(out_wire.id(), &map, wires, var_count)
}
...
pub fn collect_used_vars(
    ...
    collect_idx(out_wire.id(), &map, var_count, &mut used);
