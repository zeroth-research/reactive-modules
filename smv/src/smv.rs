    ...
    match rule {
        ...
        Rule::expr_or | Rule::expr_and => {
            ...
            let write = match &final_write {
                Some(w) => w.clone(),
                None => {
                    let w = Wire::one(*temp_index, DType::Bool);
                    *temp_index += 1;
                    w
                }
            };
        ...
        Rule::expr_factor => {
            ...
            if first.as_rule() == Rule::NOT {
                ...
                let write = match &final_write {
                    Some(w) => w.clone(),
                    None => {
                        Wire::one(*temp_index, DType::Bool);
                        *temp_index += 1;
                        Wire::one(*temp_index - 1, DType::Bool)
                    }
                };
        ...
        Rule::number => {
            ...
            let write = match &final_write {
                Some(w) => w.clone(),
                None => {
                    Wire::one(*temp_index, DType::Int);
                    *temp_index += 1;
                    Wire::one(*temp_index - 1, DType::Int)
                }
            };
            terms.push(Term::new(
    ...
    match expr_pair.as_rule() {
        ...
        Rule::expr_arith => {
            ...
            while let Some(op) = inner.next() {
                ...
                left = match op.as_rule() {
                    ...
                    Rule::MINUS => (IType::Sub, Wire::one(offset, DType::Int), combined_read),
                    _ => {
        ...
        Rule::expr_term => {
            ...
            while let Some(op) = inner.next() {
                ...
                left = match op.as_rule() {
                    ...
                    Rule::DIVIDE => (IType::Div, Wire::one(offset, DType::Int), combined_read),
                    _ => {
                        left = build_expr(op, wires, offset);
                        continue;
                    }
        ...
        Rule::expr_factor => {
            ...
            if first.as_rule() == Rule::NOT {
                ...
                (IType::Not, Wire::one(offset, DType::Bool), sub_read)
            } else {
        ...
        Rule::expr_assign => {
            ...
            (IType::Assign, Wire::none(), rhs.2.clone())
        }
