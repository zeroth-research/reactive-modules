        ...
        match r {
            ...
            PyVal::Tensor(tensor) => {
                ...
                result.push(TorchTerm::new(
                    ...
                    Wire::one(var, TorchDType::Tensor),
                    Wire::none(),
                ));
                args.push(var);
            }
...
fn wterms_to_torchterms(ctx: &mut Context, terms: &Bound<'_, PyList>) -> PyResult<Vec<TorchTerm>> {
    ...
    for item in terms {
        ...
        let write = Wire::from_iter(wargs.into_iter().map(|val| (val, TorchDType::Tensor)));
        let read = Wire::from_iter(rargs.into_iter().map(|val| (val, TorchDType::Tensor)));
...
pub(crate) fn vars_to_wiring(vals: &Bound<'_, PyList>) -> PyResult<WireTy> {
    ...
    Ok(Wire::from_iter(
        names.into_iter().map(|val| (val, TorchDType::Tensor)),
    ))
}
