...
fn build_manual_module() -> Module<DType, IType> {
    ...
    // With the new IType representation operators are unit variants; the Term
    // carries the operator and the read wire contains the operand variables.
    // let update_terms = vec![
    //     Term::new(
    //         IType::Lt,
    //         x.twin(5).unwrap(),
    //         x.union(&Wire::one(6, DType::Int)).unwrap(), // x + 1
    //     ),
    //     Term::new(
    //         IType::Cond,
    //         x.twin(5).unwrap(),
